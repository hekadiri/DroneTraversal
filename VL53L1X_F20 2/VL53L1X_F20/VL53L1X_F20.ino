...
void send_distance(uint16_t dist, uint8_t channel )           // Send ranging measurement to arduPilot
{
...
void loop()
{
 // Serial.println( LoopTime - lastlooptime_ms );                      // diagnostic print
  ...
  uint16_t dist = sensor[channel].read();           // get a value
  ...
  if (sensor[channel].ranging_data.range_status == VL53L1X::RangeValid)    // got good data :)
  {
    ...
    if ( printFlag ) printGoodData( channel, dist, frequency );
  }
  else                                            // don't got good data :(
  {
    dist = MAX_DISTANCE;                          //if bad data always set to MAX   
    if ( printFlag )
    {   
  ...
  distance[channel] = (uint16_t)(ALPHA * (float)dist + ( 1.0f - ALPHA ) * (float)distance[channel] );
